...
-- ====== Modern Theme System v3.6 [Advanced Fixed] ======
local Themes = {
    Dark = {
...
local PlaybackSpeeds = {
    ...
    x1 = {multiplier = 1.5, label = "1x"},
    x3 = {multiplier = 3, label = "3x"},
    x5 = {multiplier = 5, label = "5x"}
}
...
-- ====== Smooth Animation Controller [FIXED] ======
...
-- Status Indicator (kotak hijau berkedip)
...
titleLabel.BackgroundTransparency = 1
titleLabel.Text = "⛩️ StrideX System - ©hyunwo"
titleLabel.Font = Enum.Font.GothamBold
...
subtitleLabel.Text = "v3.6 [Advanced Fixed] • Speed Control"
...
statusBox.Size = UDim2.new(1, -24, 0, 56)
...
statusIconBg.Size = UDim2.new(0, 36, 0, 36)
statusIconBg.Position = UDim2.new(0, 10, 0.5, -18)
...
statusTitle.Size = UDim2.new(1, -58, 0, 16)
statusTitle.Position = UDim2.new(0, 52, 0, 10)
...
recordBox.Size = UDim2.new(1, -24, 0, 82)
recordBox.Position = UDim2.new(0, 12, 0, 134)
...
recordTitle.Size = UDim2.new(1, -16, 0, 14)
...
local recordBtn, recordEmoji, recordLabel = createModernButton(recordBox, "Record", "⏺️", UDim2.new(0, 158, 0, 36), UDim2.new(0, 8, 0, 36), activeTheme.Danger)
local saveBtn, saveEmoji, saveLabel = createModernButton(recordBox, "Save", "💾", UDim2.new(0, 158, 0, 36), UDim2.new(1, -166, 0, 36), activeTheme.Success)

-- Speed Control Box
local speedBox = Instance.new("Frame", contentContainer)
speedBox.Size = UDim2.new(1, -24, 0, 82)
speedBox.Position = UDim2.new(0, 12, 0, 228)
speedBox.BackgroundColor3 = activeTheme.BoxBg
speedBox.BorderSizePixel = 0

addCorner(speedBox, 12)

local speedTitle = Instance.new("TextLabel", speedBox)
speedTitle.Size = UDim2.new(1, -16, 0, 14)
speedTitle.Position = UDim2.new(0, 8, 0, 8)
speedTitle.BackgroundTransparency = 1
speedTitle.Text = "Playback Speed"
speedTitle.Font = Enum.Font.GothamBold
speedTitle.TextSize = 10
speedTitle.TextColor3 = activeTheme.TextMuted
speedTitle.TextXAlignment = Enum.TextXAlignment.Left

local speedBtnNormal, _, _ = createModernButton(speedBox, "Normal", nil, UDim2.new(0, 76, 0, 36), UDim2.new(0, 8, 0, 36), activeTheme.Success)
local speedBtn1x, _, _ = createModernButton(speedBox, "1x", nil, UDim2.new(0, 76, 0, 36), UDim2.new(0, 90, 0, 36), activeTheme.Accent)
local speedBtn3x, _, _ = createModernButton(speedBox, "3x", nil, UDim2.new(0, 76, 0, 36), UDim2.new(0, 172, 0, 36), activeTheme.Accent)
local speedBtn5x, _, _ = createModernButton(speedBox, "5x", nil, UDim2.new(0, 76, 0, 36), UDim2.new(0, 254, 0, 36), activeTheme.Accent)

local playbackBox = Instance.new("Frame", contentContainer)
playbackBox.Size = UDim2.new(1, -24, 0, 82)
playbackBox.Position = UDim2.new(0, 12, 0, 322)
...
playbackTitle.Size = UDim2.new(1, -16, 0, 14)
...
local loadBtn, loadEmoji, loadLabel = createModernButton(playbackBox, "Load", "📂", UDim2.new(0, 158, 0, 36), UDim2.new(0, 8, 0, 36), activeTheme.Accent)
local queueBtn, queueEmoji, queueLabel = createModernButton(playbackBox, "Queue", "📋", UDim2.new(0, 158, 0, 36), UDim2.new(1, -166, 0, 36), Color3.fromRGB(138, 100, 220))

local replayBox = Instance.new("Frame", contentContainer)
replayBox.Size = UDim2.new(1, -24, 0, 96)
replayBox.Position = UDim2.new(0, 12, 0, 416)
...
replayTitle.Size = UDim2.new(1, -16, 0, 14)
...
replayScroll.Size = UDim2.new(1, -16, 1, -30)
replayScroll.Position = UDim2.new(0, 8, 0, 26)
...
copyrightLabel.Size = UDim2.new(1, 0, 0, 16)
copyrightLabel.Position = UDim2.new(0, 0, 1, -16)
copyrightLabel.BackgroundTransparency = 1
copyrightLabel.Text = "© Hunwo 2025 • v3.6 [Advanced Fixed]"
...
local function updateSpeedButtons(selectedSpeed)
    local buttons = {
        Normal = speedBtnNormal,
        x1 = speedBtn1x,
        x3 = speedBtn3x,
        x5 = speedBtn5x
    }
    ...
    for speed, btn in pairs(buttons) do
        if speed == selectedSpeed then
            btn.BackgroundColor3 = activeTheme.Success
        else
...
speedBtnNormal.MouseButton1Click:Connect(function()
    currentSpeed = "Normal"
    updateSpeedButtons("Normal")
end)

speedBtn1x.MouseButton1Click:Connect(function()
    currentSpeed = "x1"
    updateSpeedButtons("x1")
end)

speedBtn3x.MouseButton1Click:Connect(function()
    currentSpeed = "x3"
    updateSpeedButtons("x3")
end)

speedBtn5x.MouseButton1Click:Connect(function()
    currentSpeed = "x5"
    updateSpeedButtons("x5")
end)
...
-- ====== Smooth Playback System [FIXED with Speed Control] ======
local function playReplay(data)
    ...
    task.spawn(function()
        local speedMultiplier = PlaybackSpeeds[currentSpeed].multiplier
        ...
        while index <= totalFrames do
            ...
            if humanoidRootPart and humanoidRootPart.Parent and character and character.Parent and currentReplayToken == token then
                ...
                pcall(function()
                    if currentSpeed == "Normal" then
                        ...
                        humanoidRootPart.CFrame = targetCFrame
                        humanoidRootPart.AssemblyLinearVelocity = velocity * speedMultiplier
                    end
                end)
            ...
            if currentSpeed == "Normal" then
                ...
                task.wait(1 / (60 * speedMultiplier))
            end
        end
...
function createReplayItem(saved, index)
    ...
    infoLabel.Size = UDim2.new(0, 90, 0, 10)
    infoLabel.Position = UDim2.new(0, 32, 0, 24)
    ...
    createEmojiBtn("▶️", UDim2.new(0, 138, 0.5, -13), activeTheme.Accent, function()
        playReplay(saved.Frames)
    end)
    
    createEmojiBtn("⏸️", UDim2.new(0, 170, 0.5, -13), activeTheme.Warning, function()
        isPaused = not isPaused
    end)
    
    createEmojiBtn("✏️", UDim2.new(0, 202, 0.5, -13), Color3.fromRGB(138, 100, 220), function()
    ...
    createEmojiBtn("🗑️", UDim2.new(0, 234, 0.5, -13), activeTheme.Danger, function()
    ...
    local upBtn = createEmojiBtn("⬆️", UDim2.new(0, 266, 0, 6), activeTheme.BoxBg, function()
        if index > 1 then
    ...
    upBtn.Size = UDim2.new(0, 26, 0, 13)
    upBtn.TextSize = 8
    
    local downBtn = createEmojiBtn("⬇️", UDim2.new(0, 266, 1, -19), activeTheme.BoxBg, function()
    ...
    downBtn.Size = UDim2.new(0, 26, 0, 13)
    downBtn.TextSize = 8
end
...
-- ====== Startup Message ======
print("╔═══════════════════════════════════════════╗")
print("║  AUTO WALK RECORDER PRO v3.6 [Advanced]  ║")
print("║   Speed Control Edition by ©hyunwo       ║")
print("╚═══════════════════════════════════════════╝")
print("")
print("✨ NEW: Playback speed control (Normal, 1x, 3x, 5x)")
print("✨ NEW: Status indicator with blink animation")
print("✨ OPTIMIZED: Compact UI design")
print("✨ OPTIMIZED: Better layout spacing")
print("✨ FIXED: Smooth animation on normal speed")
print("✨ FIXED: Fast playback for speed modes")
print("")
print("🎮 Speed Modes:")
print("   • Normal: Original recording speed")
print("   • 1x: 1.5x faster playback")
print("   • 3x: 3x faster playback")
print("   • 5x: 5x faster playback")
print("")
print("💡 Tip: Use Normal for smooth animations!")
