...
-- ====== Speed Dropdown Menu (Integrated into Control Box) ======
local currentSpeed = 1
local speedDropdownOpen = false
local dropdownBtn, dropdownList, arrowIcon

local function createSpeedDropdown(parent)
    local dropdownContainer = Instance.new("Frame", parent)
    dropdownContainer.Size = UDim2.new(1, -16, 0, 36)
    dropdownContainer.Position = UDim2.new(0, 8, 0, 158)
    dropdownContainer.BackgroundTransparency = 1
    dropdownContainer.ZIndex = 10
    
    local speedLabel = Instance.new("TextLabel", dropdownContainer)
    ...
    dropdownBtn = Instance.new("TextButton", dropdownContainer)
    dropdownBtn.Size = UDim2.new(1, -60, 1, 0)
    dropdownBtn.Position = UDim2.new(0, 55, 0, 0)
    dropdownBtn.BackgroundColor3 = activeTheme.BoxBg
    dropdownBtn.BorderSizePixel = 0
    dropdownBtn.Font = Enum.Font.GothamMedium
    dropdownBtn.TextSize = 11
    dropdownBtn.TextColor3 = activeTheme.TextPrimary
    dropdownBtn.Text = string.format("%.1fx", currentSpeed)
    dropdownBtn.AutoButtonColor = false
    
    addCorner(dropdownBtn, 8)
    addStroke(dropdownBtn, activeTheme.Border, 1)
    
    arrowIcon = Instance.new("TextLabel", dropdownBtn)
    ...
    arrowIcon.TextColor3 = activeTheme.TextMuted
    
    dropdownList = Instance.new("Frame", dropdownContainer)
    dropdownList.Size = UDim2.new(1, -60, 0, 0)
    dropdownList.Position = UDim2.new(0, 55, 1, 4)
    dropdownList.BackgroundColor3 = activeTheme.SecondaryBg
    ...
    addStroke(dropdownList, activeTheme.Border, 1)
    
    local speedOptions = {0.5, 0.75, 1, 1.25, 1.5}
    ...
    for i, speed in ipairs(speedOptions) do
        ...
        optionBtn.BackgroundColor3 = activeTheme.SecondaryBg
        optionBtn.BorderSizePixel = 0
        optionBtn.Font = Enum.Font.GothamMedium
        optionBtn.TextSize = 11
        optionBtn.TextColor3 = activeTheme.TextPrimary
        optionBtn.Text = string.format("%.2fx", speed)
        ...
        optionBtn.MouseLeave:Connect(function()
            TweenService:Create(optionBtn, TweenInfo.new(0.15), {
                BackgroundColor3 = activeTheme.SecondaryBg
            }):Play()
        end)
        
        optionBtn.MouseButton1Click:Connect(function()
            currentSpeed = speed
            dropdownBtn.Text = string.format("%.1fx", speed)
    ...
    dropdownBtn.MouseButton1Click:Connect(function()
        speedDropdownOpen = not speedDropdownOpen
        
        if speedDropdownOpen then
            dropdownList.Visible = true
            dropdownList.Size = UDim2.new(1, -60, 0, 0)
            
            TweenService:Create(dropdownList, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {
                Size = UDim2.new(1, -60, 0, #speedOptions * 32)
            }):Play()
...
-- ====== Main UI Setup [v3.9 - INTEGRATED LAYOUT] ======
...
subtitleLabel.Text = "v3.9 - Integrated Control Layout"
...
-- ====== Unified Control Box (4 Buttons + Speed Dropdown) ======
local controlBox = Instance.new("Frame", contentContainer)
controlBox.Size = UDim2.new(1, -24, 0, 202)
controlBox.Position = UDim2.new(0, 12, 0, 140)
...
controlTitle.Position = UDim2.new(0, 8, 0, 8)
...
local recordBtn, recordEmoji, recordLabel = createModernButton(controlBox, "Record", "⏺️", UDim2.new(0.48, -4, 0, 36), UDim2.new(0, 8, 0, 30), activeTheme.Danger)
local saveBtn, saveEmoji, saveLabel = createModernButton(controlBox, "Save", "💾", UDim2.new(0.48, -4, 0, 36), UDim2.new(0.52, 4, 0, 30), activeTheme.Success)
local loadBtn, loadEmoji, loadLabel = createModernButton(controlBox, "Load", "📂", UDim2.new(0.48, -4, 0, 36), UDim2.new(0, 8, 0, 72), activeTheme.Accent)
local queueBtn, queueEmoji, queueLabel = createModernButton(controlBox, "Queue", "📋", UDim2.new(0.48, -4, 0, 36), UDim2.new(0.52, 4, 0, 72), Color3.fromRGB(138, 100, 220))

-- Separator line
local separator = Instance.new("Frame", controlBox)
separator.Size = UDim2.new(1, -16, 0, 1)
separator.Position = UDim2.new(0, 8, 0, 120)
separator.BackgroundColor3 = activeTheme.Border
separator.BorderSizePixel = 0

-- Speed label
local speedSectionLabel = Instance.new("TextLabel", controlBox)
speedSectionLabel.Size = UDim2.new(1, -16, 0, 16)
speedSectionLabel.Position = UDim2.new(0, 8, 0, 130)
...
-- ====== Speed Dropdown (Inside Control Box) ======
local speedDropdown = createSpeedDropdown(controlBox)

-- ====== Replay List Box ======
local replayBox = Instance.new("Frame", contentContainer)
replayBox.Size = UDim2.new(1, -24, 0, 130)
replayBox.Position = UDim2.new(0, 12, 0, 354)
...
copyrightLabel.Text = "© Hunwo 2025 • v3.9"
...
local function applyTheme()
    ...
    if dropdownBtn then
        dropdownBtn.BackgroundColor3 = activeTheme.BoxBg
        dropdownBtn.TextColor3 = activeTheme.TextPrimary
        local stroke = dropdownBtn:FindFirstChildOfClass("UIStroke")
        if stroke then stroke.Color = activeTheme.Border end
    end
    
    if dropdownList then
        dropdownList.BackgroundColor3 = activeTheme.SecondaryBg
        local stroke = dropdownList:FindFirstChildOfClass("UIStroke")
        if stroke then stroke.Color = activeTheme.Border end
        
        for _, child in ipairs(dropdownList:GetChildren()) do
            if child:IsA("TextButton") then
                child.BackgroundColor3 = activeTheme.SecondaryBg
                child.TextColor3 = activeTheme.TextPrimary
            end
        end
    end
...
-- ====== Smooth Playback System with Speed Control ======
...
-- ====== Startup Message ======
print("╔═══════════════════════════════════════════╗")
print("║  AUTO WALK RECORDER PRO v3.9              ║")
print("║     Integrated Control Layout             ║")
print("╚═══════════════════════════════════════════╝")
print("")
print("✨ NEW: Speed dropdown inside Control Box")
print("✨ NEW: Compact unified layout")
print("✨ NEW: Cleaner UI organization")
print("✨ FIXED: All controls in one section")
print("")
print("🎮 Features:")
print("   • Record, Save, Load, Queue buttons")
print("   • Speed control (0.5x - 1.5x)")
print("   • Smooth minimize/restore animations")
print("   • Organized replay list")
print("")
print("💡 All controls are now in one place!")
