...
function AnimationController.new(character)
    ...
    self.lerpAlpha = 0.35 -- Increased for smoother movement
    self.previousVelocity = Vector3.new()
    self.stateBuffer = {}
    self.bufferSize = 8 -- Increased buffer for better stability
    self.positionBuffer = {}
...
function AnimationController:smoothMoveTo(targetCF, velocity)
    ...
    -- Position buffering for ultra-smooth movement
...
function AnimationController:detectState(velocity)
    ...
    -- FIXED: Better jump/fall detection with proper thresholds
...
subtitleLabel.Text = "Enhanced Smoothing & Jump Fix"
...
-- ====== Replay List Box (COMPACT - FIXED SIZE) ======
...
-- ====== Enhanced Recording System v4.1 ======
local function startRecording()
    ...
    recordingConnection = RunService.Heartbeat:Connect(function()
        ...
        if humanoidRootPart and humanoidRootPart.Parent and character and character.Parent then
            ...
            -- FIXED: Store vertical velocity change for better jump detection
...
-- ====== Ultra Smooth Playback System v4.1 ======
    ...
    task.spawn(function()
        ...
        while index <= totalFrames do
            ...
            if humanoidRootPart and humanoidRootPart.Parent and character and character.Parent and currentReplayToken == token then
                ...
                -- FIXED: Improved jump/fall detection during playback
                local currentVerticalVel = velocity.Y
                local verticalChange = currentVerticalVel - previousVerticalVel
                previousVerticalVel = currentVerticalVel
                
                local playbackState = frame.State
                if frame.VerticalChange then
                    if frame.VerticalChange > 40 or (currentVerticalVel > 25 and verticalChange > 0) then
                        playbackState = "Jumping"
                    elseif currentVerticalVel < -25 and verticalChange < 0 then
                        playbackState = "Falling"
                    end
                end
                ...
                pcall(function()
                    if animController then
                        ...
                        humanoidRootPart.CFrame = humanoidRootPart.CFrame:Lerp(targetCFrame, 0.35)
                        humanoidRootPart.AssemblyLinearVelocity = velocity
                    end
                end)
                
                if playbackState and playbackState ~= lastState then
                    if animController then
...
print("║   Enhanced Smoothing & Jump Fix           ║")
print("╚═══════════════════════════════════════════╝")
print("")
print("✨ NEW IN v4.1:")
print("   • Ultra-smooth playback (35% lerp)")
print("   • Fixed jump/fall detection")
print("   • Position buffering system")
print("   • Vertical velocity tracking")
print("   • Compact replay list (90px)")
print("   • Fixed copyright visibility")
print("")
print("🎮 Features:")
print("   • Manual speed control (0.1x - 5.0x)")
print("   • Improved animation smoothness")
print("   • Better jump detection in playback")
print("   • Organized compact UI")
print("")
print("💡 Enjoy smoother recordings & playback!")
