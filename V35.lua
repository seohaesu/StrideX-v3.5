...
-- ====== Enhanced Animation Controller v4.1 ======
...
function AnimationController:detectState(velocity)
    ...
    if verticalSpeed > self.jumpThreshold then
        return "Jumping"
    elseif verticalSpeed < -self.jumpThreshold then
        return "Falling"
...
local function createSpeedInput(parent)
    ...
    inputContainer.Size = UDim2.new(1, -16, 0, 52)
...
mainPanel.Size = UDim2.new(0, 380, 0, 488)
mainPanel.Position = UDim2.new(0.5, -190, 0.5, -244)
...
titleLabel.BackgroundTransparency = 1
titleLabel.Text = "StrideX System v4.1"
titleLabel.Font = Enum.Font.GothamBold
...
subtitleLabel.Text = "v4.1 - Compact & Jump Fix"
...
controlBox.Size = UDim2.new(1, -24, 0, 296)
...
replayBox.Position = UDim2.new(0, 12, 0, 374)
...
copyrightLabel.Size = UDim2.new(1, 0, 0, 10)
copyrightLabel.Position = UDim2.new(0, 0, 1, -10)
copyrightLabel.BackgroundTransparency = 1
copyrightLabel.Text = "© Hunwo 2025 • v4.1"
...
local function applyTheme()
    ...
    if speedInputBox then
        speedInputBox.TextColor3 = activeTheme.TextPrimary
        speedInputBox.PlaceholderColor3 = activeTheme.TextMuted
        if speedInputStroke then speedInputStroke.Color = activeTheme.Border end
    end
    for _, item in ipairs(replayScroll:GetChildren()) do
        if item:IsA("Frame") then item.BackgroundColor3 = activeTheme.ItemBg end
    end
end
...
minimizeBtn.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        ...
        TweenService:Create(mainPanel, TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.InOut), {
            Size = UDim2.new(0, 0, 0, 0), Position = targetPos, BackgroundTransparency = 1
        }):Play()
        ...
        TweenService:Create(mainPanel, TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
            Size = originalSize, Position = originalPosition, BackgroundTransparency = 0
        }):Play()
...
minimizedBtn.MouseEnter:Connect(function()
    TweenService:Create(minimizedBtn, TweenInfo.new(0.2), {
        Size = UDim2.new(0, 70, 0, 70), BackgroundColor3 = activeTheme.AccentHover
    }):Play()
end)

minimizedBtn.MouseLeave:Connect(function()
    TweenService:Create(minimizedBtn, TweenInfo.new(0.2), {
        Size = UDim2.new(0, 60, 0, 60), BackgroundColor3 = activeTheme.Accent
    }):Play()
end)
...
local function onCharacterAdded(char)
    ...
    if animController then animController:cleanup() end
...
local function updateStatus(state, animation, frames)
    ...
    local emojiMap = {Idle = "▶️", Walking = "🚶", Running = "🏃", Jumping = "⬆️", Falling = "⬇️", Climbing = "🧗"}
    statusIcon.Text = emojiMap[animation] or "▶️"
    local colorMap = {Recording = activeTheme.Danger, Playing = activeTheme.Accent, Paused = activeTheme.Warning, Ready = activeTheme.Success}
...
local function startRecording()
    ...
    recordingConnection = RunService.Heartbeat:Connect(function()
        ...
        if currentTime - lastRecordTime < recordInterval then return end
        lastRecordTime = currentTime
        if humanoidRootPart and humanoidRootPart.Parent and character and character.Parent then
            local cf = humanoidRootPart.CFrame
            local velocity = humanoidRootPart.AssemblyLinearVelocity
            local state = "Idle"
            if animController then state = animController:detectState(velocity) end
            ...
            if frameCount % 15 == 0 then updateStatus("Recording", state, frameCount) end
        else
            stopRecording()
        end
    end)
        ...
        while index <= totalFrames do
            ...
            if humanoidRootPart and humanoidRootPart.Parent and character and character.Parent and currentReplayToken == token then
                ...
                if frame.State and frame.State ~= lastState then
                    if animController then animController:applyState(frame.State) end
                    lastState = frame.State
                end
                if index % 5 == 0 then updateStatus("Playing", frame.State or "Idle", math.floor(index)) end
            else
                break
            end
    ...
    task.spawn(function()
        while autoQueueRunning do
            ...
            for _, r in ipairs(savedReplays) do
                if r.Selected then table.insert(queue, r.Frames) end
            end
            if #queue == 0 then
                autoQueueRunning = false
                queueLabel.Text = "Queue"
            ...
            for i, frames in ipairs(queue) do
                ...
                playReplay(frames)
                while currentReplayToken do task.wait(0.1) end
                task.wait(0.5)
            end
...
function createReplayItem(saved, index)
    ...
    createEmojiBtn("▶️", UDim2.new(0, 148, 0.5, -14), activeTheme.Accent, function() playReplay(saved.Frames) end)
    createEmojiBtn("⏸️", UDim2.new(0, 182, 0.5, -14), activeTheme.Warning, function() isPaused = not isPaused end)
    ...
    createEmojiBtn("🗑️", UDim2.new(0, 250, 0.5, -14), activeTheme.Danger, function()
        TweenService:Create(item, TweenInfo.new(0.3), {Size = UDim2.new(1, 0, 0, 0), BackgroundTransparency = 1}):Play()
        task.wait(0.3)
...
screenGui.Destroying:Connect(function()
    ...
    if animController then animController:cleanup() end
    if currentReplayToken then currentReplayToken = nil end
    autoQueueRunning = false
    isRecording = false
end)

print("╔═══════════════════════════════════════════╗")
print("║  AUTO WALK RECORDER PRO v4.1              ║")
print("║     Compact Layout & Jump Fix             ║")
print("╚═══════════════════════════════════════════╝")
print("")
print("✨ NEW IN v4.1:")
print("   • Compact speed input (saves space)")
print("   • Copyright text now visible")
print("   • Jump detection fixed in playback")
print("   • Reduced panel height (488px)")
print("")
print("🔧 FIXES:")
print("   1. Speed box reduced: 64px → 52px")
print("   2. Copyright visible at bottom")
print("   3. Jump state properly recorded/played")
print("")
print("🎮 How to use:")
print("   • Record movements (including jumps)")
print("   • Jumps now play correctly as jumps")
print("   • Type custom speed: 0.1 - 5.0")
print("")
print("💡 Jump Fix: States are recorded and replayed")
print("   exactly as they were performed!")
